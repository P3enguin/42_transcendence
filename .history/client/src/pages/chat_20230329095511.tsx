...
function Chat({ jwt_token, data }: { jwt_token: string; data: any }) {
  ...
  return (
    <>
      ...
      <div className="m-5 flex min-h-[700px] min-w-[1000px] flex-row rounded-2xl border border-neutral-300 sm:m-20 md:h-[80%]">
                ...
                data.player.friends.map((friend: any, index: any) =>(
                  <Link key={index} href={`/users/${friend.nickname}`}>
                    <img
                      ...
                      // src={`blob:http://localhost:3000/${data.player.avatar}`}
