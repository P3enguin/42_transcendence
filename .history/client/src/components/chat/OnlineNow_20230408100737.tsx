...
function OnlineNow ({player, avatar}: any)
{
  ...
  // setLoading(false);
  const picture = "http://localhost:8000/avatars/"+ avatar} 
    return (
      ...
      <Link href={`/users/${player}`}>
        <img className="h-14 w-14 md:w-16 md:h-16 cursor-pointer rounded-full border" src={} alt="Avatar" width={12} height={12} />
      </Link> 
      </div>
