...
function OnlineNow ({player, avatar, key}: any)
{
  ...
  // setLoading(false);
  const picture = process.env.NEXT_PUBLIC_BACKEND_HOST + "localhost:8000/avatars/"+ avatar;
  console.log(player);
    ...
    return (
      ...
      <Link href={`/users/${player}`}>
        <Image className="h-14 w-14 md:w-16 md:h-16 cursor-pointer rounded-full border" src={picture} alt="Avatar" width={12} height={12} />
      </Link> 
      </div>
